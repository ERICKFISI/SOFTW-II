<script>
    $(document).ready(function () {
        ...
        $('#idproducto').change(function () {
            $.post('<?= base_url(); ?>/ingreso/getPrecioThisProducto', 'idproducto=' + $(this).val(), function (data) {
                if (data.resp == 1) {
        ...
        agregarProductoCarrito = function () {
            ...
            $.post('<?= base_url(); ?>/ingreso/verificarProductoEnCarrito', 'idproducto=' + idproducto, function (dt) {
                if (dt.resp == 1) {
        ...
        agregarProducto = function () {
            ...
            $.post('<?= base_url(); ?>/ingreso/setProductoAlCarrito', 'idproducto=' + idproducto + '&cantidad=' + cantidad
                    + '&preciounidad=' + preciounidad + '&subtotal=' + subtotal + '&descripcion_producto=' + descripcion_producto, function (data) {
                        formar_tabla(data);
                    }, 'json');
        };
        eliminar = function (id) {
            $.post('<?= base_url(); ?>/ingreso/eliminarProductoAlCarrito', 'idproducto=' + id, function (data) {
                if (data.resp == 1) {
                    formar_tabla(data);
                }
            }, 'json');
...
<div class="">
    <div class="page-title">
        <div class="title_left">
            <h3>Editar Ingreso de Almacén</h3>
        </div>
    </div>
    <div class="col-md-3-6 ">
        <div class="x_panel">
            <div class="x_content">
                <br />
                <form class="form-horizontal form-label-left h6" action="<?= base_url(); ?>/ingreso/update/<?= $ingreso['idingreso'] ?>" method="POST">
                        ...
                        <div class="col-md-9 col-sm-9 ">
                            <select class="form-control" id="idtipoingreso" name="idtipoingreso" required="">
                                ...
                                foreach ($tipoingreso as $key => $value) {
                                    ...
                                    if ($value['idtipoingreso'] == $ingreso['idtipoingreso']) {
                                        $html .= '<option value="' . $value['idtipoingreso'] . '" selected>' . $value['tipoingreso'] . '</option>';
                                    } else {
                    ...
                    <div class="form-group row ">
                        ...
                        <div class="col-md-9 col-sm-9 ">
                            <input type="text" class="form-control"  id="descripcioningreso" name="descripcioningreso" value="<?php echo $ingreso['descripcioningreso'] ?>" required>
                    </div>           
                    </div>           
