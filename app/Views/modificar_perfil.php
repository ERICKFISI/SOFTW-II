                    ...
                    <div class="page-title">
                        ...
                        <div class="col-md-3-6 ">
                            <div class="x_panel">
                                ...
                                <div class="x_content">
                                    <br />
                                    <form class="form-horizontal form-label-left h6" action="visualizarperfil/update/<?= $perfiles['idperfil']; ?>" method="POST">

                                        <div class="form-group row ">
                                            <label class="control-label col-md-3 col-sm-3 ">Nombre del perfil
                                            </label>
                                            <div class="col-md-9 col-sm-9 ">
                                                <input type="text" class="form-control" name="nombreperfil" required value="<?= $perfiles['nombre']; ?>">
                                            </div>
                                        </div>
					<?php foreach ($modulos as $modulo => $key){?>
                        <?php if ( $key['idmodulo'] == $permisos[$modulo]['idmodulo'] && $permisos[$modulo]['idperfil'] == $perfiles['idperfil'] )  {?>
					<div class="checkbox">
						<label>
							<input type="checkbox" checked = "checked" value="<?=  $key['idmodulo'];?>" id="<?= $key['idmodulo'];?>" name="checks[]"><?= $key['nombre']; ?>
						</label>
					</div>
                <?php }else {?>
                   <div class="checkbox">
                        <label>
                            <input type="checkbox" value="<?=  $key['idmodulo'];?>" id="<?= $key['idmodulo'];?>" name="checks[]"><?= $key['nombre']; ?>
                        </label>
                    </div>
					<?php } }?>
                                        <!--
